  ...
  security_group_rules = merge(
    {
      text_generation_inference = {
        ...
        description              = "NFS ingress from TextGenerationInference"
  ...
  access_points = merge(
    {
      text_generation_inference = {
        posix_user = {
          gid = var.text_generation_inference_gid
          uid = var.text_generation_inference_uid
        }
        root_directory = {
          path = local.root_directory_path
          creation_info = {
            owner_gid   = var.text_generation_inference_gid
            owner_uid   = var.text_generation_inference_uid
            permissions = "0750"
          }
        }
      }
